...
first_last_orders['months_diff'] = (first_last_orders['last_order_date'] - first_last_orders['first_order_date']).apply(lambda x: x.n) + 7
...
for row in first_last_orders.itertuples():
    for i in range(row.months_diff + 1):
        for j in range(row.months_diff + 1 - i):
            if row.first_order_date + i + j > highest_month:
                continue
            try:
        ...
        html.Div([
            ...
            html.Div([
                ...
                dcc.RangeSlider(
                    ...
                    max=24,
                    value=[1, 6],
                    marks={i: f'{i} мес.' for i in range(25)},
                    step=1,
...
def create_cohort_analysis(df):
    ...
    retention = cohort_counts.div(start_values[:-1], axis=0) * 100
    retention = retention.replace(0, np.nan)
    return retention.round(1)


def register_callbacks_ca(app):
    @app.callback(
