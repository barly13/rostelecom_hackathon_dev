...
rfm_analysis = pd.read_csv(f"{current_dir}/rfm_data/rfm_analysis.csv")
rfm_analysis["time"] = rfm_analysis.last_purchase_date.apply(lambda x: (today - datetime.strptime(x, '%Y-%m-%d %H:%M:%S')).total_seconds())
rfm_analysis["count"] = 1 #rfm_analysis.time.apply(get_probability)
rfm_analysis = rfm_analysis[["rfm_cell", "count"]]
rfm_analysis = rfm_analysis.groupby("rfm_cell").sum()
...
def create_tensor_cube(rfm):
    ...
    for i in range(3):
        for j in range(3):
            for k in range(3):
                ...
                y.append(j)
                z.append(k)
                values.append(int(rfm[(rfm.latency == i+1) & (rfm.freq == j+1) & (rfm.money == k+1)][["count"]].iloc[0]))
    ...
    fig.add_trace(go.Scatter3d(
        ...
        text=[f'Покупателей в группе: {val}' for val in values],
        hoverinfo='text'
    ))
...
def create_heatmap(rfm, ax):
    ...
    form_string = "{}, {}, Кол-во покупателей в группе: {}"
    first = []
    second = []
    x_label = ""
    y_label = ""
    if ax == "latency": # по давности объединение
        ...
        x_label = "Средний чек"
        y_label = "Частота"
        rfm = rfm.groupby(["freq", "money"])[["count"]].sum()
        for money in (1, 2, 3):
            for freq in (1, 2, 3):
                if (freq, money) in rfm.index:
                    tensor[freq-1, money-1] = int(rfm.loc[freq, money])
    elif ax == "frequency": # по частоте объединение
        first = ["Недавние", "Средней давности", "Давние"]
        second = ["Высокий чек", "Средний чек", "Низкий чек"]
        x_label = "Средний чек"
        y_label = "Давность"
        rfm = rfm.groupby(["latency", "money"])[["count"]].sum()
        for money in (1, 2, 3):
            for latency in (1, 2, 3):
                if (latency, money) in rfm.index:
                    tensor[latency-1, money-1] = int(rfm.loc[latency, money])
    elif ax == "money": # по чеку объединение
        first = ["Недавние", "Средней давности", "Давние"]
        second = ["Частые", "Средней частоты", "Редкие"]
        x_label = "Частота"
        y_label = "Давность"
        rfm = rfm.groupby(["latency", "freq"])[["count"]].sum()
        for freq in (1, 2, 3):
            for latency in (1, 2, 3):
                if (latency, freq) in rfm.index:
                    tensor[latency-1, freq-1] = int(rfm.loc[latency, freq])
...
def register_callbacks_rfm(app):
    @app.callback(
